...
\begin{proof} Let \(m\) be a vector in the range of \(P\), that is, \(m= Px\) for some \(x\). Then \(Pm = P^2 x = Px = m\). 
...
This can be factorized as \[p(S) = (S - \alpha_1I) (S - \alpha_2 I) \cdots (S - \alpha_n I)\] Let \(B_j = S - \alpha_jI\). Then \(p(S) = \prod_j B_j\)
...
We get \(q(z) = (1 - \conj{\alpha_1}z) (1 - \conj{\alpha_2}z) \cdots (1 - \conj{\alpha_nz})\). Also, \(q(S) = (I - \conj{\alpha_1}S) (I - \conj{\alpha_2}S) \cdots (I - \conj{\alpha_n}S)\). Let \(C_j= I -  \conj{\alpha_j} S\). Then \(q(S) = \prod_j C_j\).
...
\begin{theorem} [Schur-Cohn] The polynomial \(p\), it will have \(k\) roots inside the circle, and \(n-k\) roots outside the circle iff \(k\) eigenvalues of \(H\) are positive and \(n-k\) are negative. \end{theorem}
...
Note that \(I - \adj{S} S\) is a posititve definite matrix. If \(\abs{\alpha_1} < 1\), then root of the linear polynomial lies within the unit circle. Also note that \(H\) has one negative eigenvalue. Similarly, if \(\abs{\alpha_1} > 1\), then the root of the linear polynomial lies outside the unit circle, and the eigenvalue of \(H\) is positive. This shows that the Schur-Cohn theorem is true for \(n = 1\). We will now extend the proof for all \(n\). 
...
\begin{equation*} \begin{split}
\adj{C_j} C_j - \adj{B_j} B_j & = (1 - \abs{\alpha_j}^2) (1 - \adj{S} S) \\ 
& = (1 - \abs{\alpha_j}^2) e_1 \adj{e_1}
\end{split} \end{equation*}
