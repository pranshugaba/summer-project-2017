...
The adjoint of a matrix \(A \in \mathbb{C}_n\) is the matrix obtained by taking its transpose, followed by taking the complex conjugate of every element. The adjoint of matrix \(A\) is denoted by \(A^*\). If the \(ij^{\text{th}} \) of \(A\) is \(a_{ij}\), then the \(ij^{\text{th}}\) entry of \(A^*\) is \(\conj{a_{ji}}\). We see that \(A^*\) is a linear transformation. The adjoint satisfies \(\langle Ax, y \rangle = \langle x, A^*y \rangle\). 
...
\begin{lemma} If \(A \in \mathbb{M}_n (\mathbb{C})\) and \(\langle Ax, x \rangle \in \mathbb{R}\) for every \(x\), then \(A = A^*\). \end{lemma}
