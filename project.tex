...
%\begin{equation*}\begin{split}
% \lambda_k &= \min_{w_1, \ldots , w_{n-k} \in \mathbb{C}^n} \left \{ \max_{\substack{x \neq 0, x\in \mathbb{C}^n \\ x \perp w_1, \ldots , w_{n-k}}} \frac{\inp{Ax}{x}}{\inp{x}{x}} \right \}\\
%& = \max_{w_1, \ldots , w_{k -1} \in \mathbb{C}^n} \left \{ \min_{\substack{x \neq 0, x\in \mathbb{C}^n \\ x \perp w_1, \ldots , w_{k - 1}}} \frac{\inp{Ax}{x}}{\inp{x}{x}} \right \}
%\end{split} \end{equation*}
% \end{theorem}

%For the first equality,

% If \(x \neq 0\), then by Spectral theorem, \[\frac{\inp{Ax}{x}}{\inp{x}{x}} = \frac{\inp{U \Lambda \adj{U} x}{x}}{\inp{\adj{U} x}{ \adj{U} x}} =  \frac{\inp{\Lambda \adj{U} x}{ \adj{U} x}}{\inp{\adj{U}x}{\adj{U}x}} \text{ and }  \{ \adj{U} x : x \neq 0\}  = \{x \in \mathbb{C}^n : x \ne 0 \}\]

%Since \(U\) is a unitary matrix, its columns form an orthonormal basis. 


%Let \(y = \adj{U} x\). Thus if \(w_1, \ldots , w_{n-k}\) are given, then 


%Now we prove the reverse inequality. Let \(w_1 = x_n , \ w_2 = x_{n-1}, \ \ldots , \ w_{n-k} = x_{k + 1}\)
...
This algorithm fails when \(\conj{\alpha}\) and \(\frac{1}{\alpha}\) are both roots of the polynomial for some \(\alpha \in \mathbb{C}\). All the points on the unit circle will satisfy this condition \(p(\conj{\alpha}) = p(\frac{1}{\alpha}) = 0\). In such a case, \(H\) becomes singular and the algorithm fails.  To avoid this, one workaround is to compute the eigenvalues at a circle of a different radius. If the polynomial does not have repeated zeros, then another way is to fix this is to apply the Schur-Cohn test on the derivative of \(p\).
 ...
 More different ways to fix this  are presented in these papers: %insert references here.
